package cn.xiaojianzheng.xiaoxin.autoit;
...
public class AutoItX {
    ...
    static {
        ...
        if (System.getProperty("sun.arch.data.model").contains("32")) {
            jacobFile = "jacob-1.20-x86.dll";
            autoItX3File = "AutoItX3.dll";
        } else {
            jacobFile = "jacob-1.20-x64.dll";
            autoItX3File = "AutoItX3_x64.dll";
        }

        File autoItX3DllFile = FileUtil.newFile(autoItX3File);
        if (!autoItX3DllFile.exists()) {
            InputStream resourceAsStream = ResourceUtil.getStream("dll/" + autoItX3File);
            FileUtil.writeFromStream(resourceAsStream, autoItX3DllFile);
        }
        String result = RuntimeUtil.execForStr("cmd", "/c", "regsvr32", autoItX3DllFile.getAbsolutePath());
        log.info("regsvr32 {} => {}", autoItX3File, result);

        File jacobDllFile = FileUtil.newFile(jacobFile);
        if (!jacobDllFile.exists()) {
            InputStream resourceAsStream = ResourceUtil.getStream("dll/" + jacobFile);
            FileUtil.writeFromStream(resourceAsStream, jacobDllFile);
        }
        System.setProperty(LibraryLoader.JACOB_DLL_PATH, jacobDllFile.getAbsolutePath());
    }
